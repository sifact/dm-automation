...
const AutomationList = ({ platform }: { platform: INTEGRATIONS }) => {
  const { data, isLoading, error } = useQueryAutomations(platform);
  ...
  const automationData = data?.data || [];
  const displayData =
    latestVariable && latestVariable?.variables && latestVariable.variables.platform === platform ? [latestVariable.variables, ...automationData] : automationData;
  ...
  return (
    <div className="flex flex-col gap-y-3">
      {displayData.map((automation) => (
        <Link
          href={`${pathname}/${automation.id}?platform=${platform}`}
          key={automation.id}
